...
// 	datepickerSet('eStartDate','eEndDate');
	  ...
	  $('table[role="grid"].gridjs-table th:nth-child(1)').css('width', '100px'); //       <th >NO.</th>    
	  ...
	  $('table[role="grid"].gridjs-table th:nth-child(4)').css("width", "220px"); //    <th >담당자</th>    
	  ...
	  $('table[role="grid"].gridjs-table th:nth-child(5)').css("width", "160px"); //       <th >담당자연락처</th> >
	  ...
	  $('table[role="grid"].gridjs-table th:nth-child(6)').css('width', '180px'); //       <th >담당자메일</th>  
	  ...
	  $('table[role="grid"].gridjs-table th:nth-child(7)').css('width', '180px'); //       <th >주사업자</th>   
	  $('table[role="grid"].gridjs-table th:nth-child(8)').css('width', '160px'); //       <th >사업PM</th>   
	  $('table[role="grid"].gridjs-table th:nth-child(9)').css('width', '160px'); //       <th >PM연락처</th>  
	  $('table[role="grid"].gridjs-table th:nth-child(10)').css('width', '180px'); //     <th >PM이메일</th>  
	...
	<div class="content_up">
		...
		<div class="tbl_top">
			<ul class="tbl_top_left" >
				<li> 사업명:
					<input name="eSearchWord1" type="text" style="width:150px;"  id="eSearchWord1" value="${mesOutputVO.eSearchWord1}" onKeydown="if(event.keyCode == 13){fn_guestList(1);}" maxlength="50" />
				</li>
				<li>
	           		<span>사업시행연도:</span>
		       		<input type="text" id="eStartDate" name="eStartDate" class="inp_color"  style="width:100px; text-align: center;" value="${mesOutputVO.eStartDate}" readonly />
<%-- 		           	~ <input type="text" id="eEndDate" name="eEndDate" class="inp_color"  style="width:100px; text-align: center;" value="${mesOutputVO.eEndDate}" readonly /> --%>
				...
				<li> 담당자:
					<input name="eSearchWord2" type="text" style="width:150px;"  id="eSearchWord2" value="${mesOutputVO.eSearchWord2}" onKeydown="if(event.keyCode == 13){fn_guestList(1);}" maxlength="50" />
				</li>
				<li> 사업 PM:
					<input name="eSearchWord3" type="text" style="width:150px;"  id="eSearchWord3" value="${mesOutputVO.eSearchWord3}" onKeydown="if(event.keyCode == 13){fn_guestList(1);}" maxlength="50" />
				</li>
<!-- 				<li> 진행상태: -->
 					...
 					<tr onclick="incomView('${outputList.eProjectNum}');"> 
						...
						<td onclick="event.cancelBubble = true;"> 
							...
							<c:if test="${outputList.sSignStatus eq '승인' || outputList.sSignStatus eq '제외'}">
								<c:if test="${staffVO.kStaffAuthWriteFlag eq 'T'}">			
									<c:if test="${outputList.kStaffKey eq staffVO.kStaffKey}">
			 							<a class="mes_btn" onclick="eDeliverable_update(${outputList.eProjectNum});">산출물등록</a> 
		 							</c:if>
	 							</c:if> 
							</c:if>
						</td>
 						...
 						<c:if test="${outputList.sSignStatus eq '승인' || outputList.sSignStatus eq '제외'}">
 							<c:if test="${staffVO.kStaffAuthWriteFlag eq 'T'}">		
 								<c:if test="${outputList.kStaffKey eq staffVO.kStaffKey}">
 								<a class="mes_btn" onclick="eReport_update(${outputList.eProjectNum});">보고서 등록</a> 
 								</c:if>
 							 </c:if>
						</c:if>
						</td>
...
<!-- 					<a onclick="excelDown()">엑셀 다운로드</a> -->
					...
					<li>프로젝트 정보는 프로젝트관리에서 등록/수정 가능합니다.
<!-- 				    	<a onclick="go_insert();"> -->
<!-- 			    			등록 -->
<!-- 			    		</a> -->
				   </li>
