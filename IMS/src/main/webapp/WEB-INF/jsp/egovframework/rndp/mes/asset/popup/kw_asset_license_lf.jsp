...
function fn_guestList(pageNo) {
	...
	document.frm.action = "/mes/asset/kw_asset_License_lf.do";
	document.frm.submit();
}
...
<form id="frm" name="frm" method="post" action="/mes/asset/kw_asset_License_lf.do">
	...
	<input type="hidden" id="eUsedLicenseQuantity" name="eUsedLicenseQuantity" value="${mesAssetVO.eUsedLicenseQuantity}">
	
	<div class="pop_head">
		<div id="pop_head">
			<div class="tit">
				<h3>대상장비 조회 <c:if test="${not empty mesAssetVO.eUsedLicenseQuantity}">&lt;총 등록된 수량:${mesAssetVO.eUsedLicenseQuantity}건&gt;</c:if></h3>
			</div>
...
<!-- 						<th style="width:10%;">자산관리</th> -->
		        ...
		        <tbody>
					<c:forEach var="list" items="${assetList}" varStatus="i">
						<tr >
<%-- 						style="cursor: pointer;" onclick="selectValue('${i.index}');" --%>
							<td>
