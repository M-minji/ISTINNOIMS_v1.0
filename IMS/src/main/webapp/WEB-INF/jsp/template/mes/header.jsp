...
.header { text-align:center;left:0; height: 50px;}
.header > .top-bar { position:relative;display: flex; justify-content: space-between; align-items: center; background: #fff; border-bottom: 2px solid #e5e5e5;}
.header > .top-bar > .logo { top:0;left:0;}
.header > .top-bar > .logo >a >img{    padding: 0 30px; width: 200px;}
...
/* 사이드바 스타일 설정 */
       #side-nav {
        ...
        #contents {
            margin-left: 200px; /* 사이드바 너비에 맞춰 조정 */
        }
        .gnb_menu {
            list-style: none;
            padding: 0;
...
<!-- 		</div>  -->
		<div class="logo">
		      <c:if test="${staff.comLogImg ne '' && staff.comLogImg ne null}">
			    	<a href="/mes/main.do">
						<img src='/cmm/fms/getImage.do?atchFileId=${staff.comLogImg}&fileSn=0'  alt="로고" onerror="this.src='/images/images/innost_error.png'"/>						
					</a>
				</c:if> 
				<c:if test="${staff.comLogImg eq ''}">
					<a href="/mes/main.do">
						<img src='/images/images/innost_error.png'  alt="로고" />
					</a>
				</c:if>    
			</div>
	</div>
        ...
        <div class="gnb">   
            <ul class="gnb_menu">
                <c:forEach var="item" items="${mesTopMenu}" varStatus="i">
                    <li>
                        <a href="#" class="menu_item <c:if test="${item.root eq root}">active</c:if>">  
                        ...
                        <ul class="gnb_sub" <c:if test="${item.root eq root}"> style="display: block;"</c:if>>
                            <c:forEach var="sub" items="${mesAllRefMenu}" varStatus="j">
                                <c:if test="${item.key eq sub.ref}">
                                    <li id='${sub.key}'  style='<c:if test="${sub.key eq key}">background-color:#DDE3E9;</c:if>'   >    
		...
		<div id="m_header">
			...
			<div class="m_left_top">	
				<ul class="m_left_top_bottom">
					...
					<li><a><c:out value='${staff.kStaffName}' />님</a></li>
					<li><a href="/mes/logout.do">LOGOUT</a></li>
					<li class="close">
						<a href="#">
